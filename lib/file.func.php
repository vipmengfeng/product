...
if(!function_exists('file_put_contents')) {
	...
	function file_put_contents($file, $string, $append = '') {
		$mode = $append == '' ? 'wb' : 'ab';
		$fp = fopen($file, "wb") or exit("Can not open $file");
		flock($fp, LOCK_EX);
...
function file_copy($from, $to) {
	...
	if(is_file($to) && MXphp_CHMOD) @chmod($to, MXphp_CHMOD);	
	if(strpos($from, DT_PATH) !== false) $from = str_replace(DT_PATH, DT_ROOT.'/', $from);
	if(@copy($from, $to)) {
		if(MXphp_CHMOD) @chmod($to, MXphp_CHMOD);
		return true;
	} else {
		return false;
	}
...
function file_put($filename, $data) {
	...
	if(@$fp = fopen($filename, 'wb')) {
		...
		flock($fp, LOCK_UN);
		fclose($fp);
		//if(MXphp_CHMOD) @chmod($filename, MXphp_CHMOD);
		return $len;
	} else {
		return false;
	}
...
function dir_create($path) {
	...
	if(DT_CACHE != DT_ROOT.'/file/cache' && strpos($path, DT_CACHE) !== false) {
		...
		for($i = 0; $i < $max; $i++) {
			...
			@mkdir($cur_dir);
			if(MXphp_CHMOD) @chmod($cur_dir, MXphp_CHMOD);
...
function dir_copy($fromdir, $todir) {
	...
	foreach($list as $v) {
		...
		if(is_file($path) && !is_writable($path)) {
			if(MXphp_CHMOD) @chmod($path, MXphp_CHMOD);
		}
		if(is_dir($v)) {
		    dir_copy($v, $path);
		} else {
