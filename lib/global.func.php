...
function template($template = 'index', $dir = '') {
	...
	$to = $dir ? ROOT_DIR.'/caches/tpl/'.$dir.'-'.$template.'.php' : ROOT_DIR.'/caches/tpl/'.$template.'.php';
	
	$isfileto = is_file($to);
	
	if($conf['template_refresh'] || !$isfileto) {
		if($dir) $dir = $dir.'/';
        $from = ROOT_DIR.'/view/default/'.$dir.$template.'.html';
		if($conf['template'] != 'default' && !is_file($from)) {
			$from = ROOT_DIR.'/view/'.$dir.$template.'.html';
		}
...
function cache_write($file, $string, $dir = '') {
	if(is_array($string)) $string = "<?php defined('IN_MXphp') or exit('Access Denied'); return ".strip_nr(var_export($string, true))."; ?>";
	$file = $dir ? DT_CACHE.'/'.$dir.'/'.$file : DT_CACHE.'/'.$file;
...
function logs($way,$method,$get){
	...
	if($way == "add"){//如果你是添加信息
	if($method == "member"){
		$sql="SELECT cusid FROM crm_user  where username='$get'";
	}elseif($method == "customer"){
		$sql="SELECT id FROM crm_customer  where cusname='$get'";
	}else{
	  ...
	  $logs[$k]['time']=$time;
		if($v['way'] == "add"){
		   if($v['method'] == "customer"){
			 $way="添加了客户";
			 $sql="SELECT cusid,cusname FROM   crm_customer  where cusname='$v[handle]'";
			 $re=$db->get_one($sql);
			 $return="<a href=''>$re[cusname]</a>";
		}elseif($v['method'] == "member"){
			$way="添加了用户";
            $sql="SELECT cusid,cusname FROM  crm_user  where id='$v[handle]'";
			$re=$db->get_one($sql);
			$return="<a href=''>$re[cusname]</a>,";			
		}else{
			$way="添加了角色";
